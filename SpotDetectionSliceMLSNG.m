function [SpotParameters, SpotDetectionInfo] = SpotDetectionSliceMLSNG(Ialigned, ZFParameters)


%Version SpotDetectionLink3
% correctiong for color and index 255 vs 256
%VersionSpotDetectionSliceMLSNG
% computed prdataset from 
...
%}
%{
    Ialigned = AlignedSlice;
    cmbr = INFO.BrainSegmentationInfo(k1).BrainEdge;
    ZFParameters = obj.ZFParameters;
...
%TODO: find out how hard thresholdig works
...
MultiProductTh = cell(1,numel(Ialigned));
Regions0 = cell(1,numel(Ialigned));
...
DistanceSpotMap = cell(1,numel(Ialigned));
DistanceBackgroundMap = cell(1,numel(Ialigned));
for k3 = 1:numel(Ialigned)
...
for k1 = 1:numel(Regions1)
    ...
    if (sbb(1)+sbb(3) > size(MultiProduct,2)) || (sbb(2)+sbb(4) > size(MultiProduct,1))
        sbb = sbb + [0,0,-2,-2];
    end
    if (sbb(1) <= 0)|| (sbb(2) <= 0)
        sbb = sbb + [ 2, 2,-2,-2];
...
%% discrimination is moved to a separate function
%{
