...
fs = zfinput(strcmp({zfinput.stage},'imageinfo')); 
%fsv = [fs.value];
for k = 1:numel(fs)
    eval([fs(k).name,'_fs=',num2str(fs(k).value)]);
end
% levels_fs =
% iterations_fs =
% scale_Fs =
...
par.levels = levels_fs;
par.iterations = iterations_fs; %iterations per level
...
for k = 1:ni
    ...
    % store bit info
    info = imfinfo([FolderPath,'/',imageinfo(k).name]);
    ...
    IS{k} = imcomplement(imresize(slice,scale_fs));
    if k>=2
        [warp(k,:),corcoef(k)] = ...
    ...
    if corcoef(k) <= threshold_fs || isnan(corcoef(k)) || modulusw(k) >= threshold2_fs
        nextstack(k) = 1;
    else
        nextstack(k) = 0;
    end
